...
public class ChatbotService {
    ...
    private List<Document> searchDocuments(String query) {
        ...
        List<Document> documents = documentRepository.searchDocuments(query, pageable);
        ...
        if (documents.size() < 2) {
            ...
            for (String keyword : keywords) {
                if (keyword.length() > 3) {  // Only use meaningful keywords
                    List<Document> additionalDocs = documentRepository.searchDocuments(keyword, pageable);
